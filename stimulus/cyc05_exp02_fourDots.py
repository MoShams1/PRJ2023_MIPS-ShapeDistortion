...
nloc = 4  # number of probe locations
...
date = sfc.get_date()
time = sfc.get_time()
output_file_name = f"exp02_fourDots_{subID}_{date}_{time}.json"
...
stm_array = np.repeat(['FG', 'FE'], 40)
assert (stm_array.size == ntrs)
probe_x_array = np.tile(np.repeat([-.75, 0, 0.01, .75], 10), 2)
assert (probe_x_array.size == ntrs)
dir_array = np.tile(np.repeat([-1, 1], 5), 8)
