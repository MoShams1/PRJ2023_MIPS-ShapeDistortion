...
for itrial in range(N_TRIALS):
    ...
    while loop_flag:
        for iori in ori_array:
            ...
            vline_x = mouse.getPos()[0] / mouse_dsf + hline_x_offset

            # print(hline_x, hline_y)

            if vline_x < -hline_size / 2:
                vline_x = -hline_size / 2
            if vline_x > hline_size / 2:
                vline_x = hline_size / 2
            ...
            if iori == flash_at_ori:
                ...
                vline.pos = vline_x, hline_y - (vline_size / 2)
                vline2.pos = vline_x, hline_y - (vline_size / 2)
                vline2.draw()
                vline.draw()
                # set pos and draw horizontal line
                hline2.draw()
    ...
    print(f'PSE: {np.round(vline_x/(hline_size/2), 2)}')
    ...
    trial_dict = {'trial_num': itrial + 1,
                  ...
                  'pse_x': [np.round(vline_x/(hline_size/2), 2)]}
