...
%% scatter bar all five conditions

figure('units','inches','outerposition',[1 1 13 8])
...
xs = scatterbar_median(data_mat,.4,4,50,.1,.05);
errorbar(1:5, median(data_mat), MAD(data_mat), ...
    'o','color','k','linewidth',2,'marker','none')

plot(xs', data_mat', 'color', .6 * ones(1,3))
...
set(gca,'Position', [.05, .05, .3, .3])

%% plot scatter (version 2)

% figure('units','inches','outerposition',[1 1 4 5])
% hold on
% 
% data_mat = [
%     distortion_observed_cross_disc,...
...
%     distortion_observed_frame_frame];
% 
% plot(xs', data_mat', 'color', .7 * ones(1,3), 'linewidth',.7)
% 
% errorbar(1:5, median(data_mat), MAD(data_mat), ...
%     'o-','markerfacecolor','k','markeredgecolor','k', 'markersize',7, ...
%     'color','k','linewidth',2)
% 
% xticks(1:5)
% xticklabels({ ...
...
data_mat_translation = mean(data_mat(:,3:5),2);
...
[delta, ~, p, W, z, r] = signrank_full(data_mat_translation, data_mat_rotation);
fprintf([ ...
...
data_mat_straight = mean(data_mat(:,[2,4,5]),2);
...
scatter_panel_position = get(gca,'Position');
...
data_mat_trajectory_diff = data_mat_curved - data_mat_straight;
xs = scatterbar_median(data_mat_trajectory_diff);
errorbar(1, median(data_mat_trajectory_diff), MAD(data_mat_trajectory_diff), ...
    'o','color','k','linewidth',2,'marker','none')
...
[delta, ~, p, W, z, r] = signrank_full(data_mat_straight, data_mat_curved);
fprintf([ ...
    '\n <Distortion difference>' ...
    '\n -----------------------' ...
    '\n median difference = %4.2f'...
...
set(ax1,'Position', [.08 .5-.5/2 .35, .5]);
