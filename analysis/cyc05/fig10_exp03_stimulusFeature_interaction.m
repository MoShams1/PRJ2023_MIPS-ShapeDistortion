...
%%
figure('units','inches','outerposition',[0 0 3.3 4])

hold on
errorbar(1:2, ...
    [median(data_mat(:,4)), median(data_mat(:,2))], ...
    [MAD(data_mat(:,4)), MAD(data_mat(:,2))], ...
    's--', 'markerfacecolor','none','color','k','linewidth',1,'markersize',8)
errorbar(1:2, ...
    [median(data_mat(:,3)), median(data_mat(:,1))], ...
    [MAD(data_mat(:,3)), MAD(data_mat(:,1))], ...
    'o-', 'markerfacecolor','k','color','k','linewidth',1,'markersize',8)
...
legend Squared Circular location northwest
