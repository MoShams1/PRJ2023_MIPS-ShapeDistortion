...
for isubj = 1:numel(all_files)
    ...
    pse_LF(isubj,1) = mean(pse_nrm(typ<0));
    pse_RF(isubj,1) = mean(pse_nrm(typ>0));

end

%% plot

x_labels = {'Left fixation', 'Right fixation'};
...
data_cell = mat2cell(data_mat, size(data_mat,1), ones(1,2));

figure('units','inches','outerposition',[1 1 6 7])
